...
- It's super old. It's like the iPhone, which was the first smartphone. It was great, but now there are better alternatives. React is the same. It was great, but now there are better alternatives. Vue 3 is super modern and it's much easier to use.
...
export default function Counter() { // You have to export and define a function
  ...
  return (
    <div>
      ...
      setCounter(c => c + 1)}>Increment</button>
    </div>
  );
}
```
